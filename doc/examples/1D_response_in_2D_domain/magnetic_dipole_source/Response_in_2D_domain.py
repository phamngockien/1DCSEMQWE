...
#Get the data
#In this example, Bx, Bz, and Ey are non-zero (see the response CSV file)
...
args = {'cmap': 'viridis', 'levels': 500, 'vmin': -20, 'vmax': -6}

# The first plot is for real E(0,Ey,0)
ax1 = fig.add_subplot(2, 2, 1)  
ax1.set_title('real E',fontsize=9)
#amplitude of the real E field
cp = plt.contourf(x, z, np.log10(np.abs(Ey_re)))
...
# The second plot is for imaginary E(0,Ey,0)
ax2 = fig.add_subplot(2, 2, 2)  
ax2.set_title('imaginary E',fontsize=9)
#amplitude of the imaginary E field
cp = plt.contourf(x, z, np.log10(np.abs(Ey_im)))
...
# The third plot is for real B(Bx,0,Bz)
ax3 = fig.add_subplot(2, 2, 3)  
ax3.set_title('real B',fontsize=9)
#amplitude of the real B field
cp = plt.contourf(x, z, np.log10(np.sqrt(Bx_re**2 + Bz_re**2)))

#normalize the real B vectors      
u = Bx_re/np.sqrt(Bx_re**2 + Bz_re**2)
v = Bz_re/np.sqrt(Bx_re**2 + Bz_re**2)
...
cp = plt.contourf(x, z, np.log10(np.sqrt(Bx_im**2 + Bz_im**2)))

#normalize the imaginary E vectors      
u = Bx_im/ np.sqrt(Bx_im**2 + Bz_im**2)
v = Bz_im/ np.sqrt(Bx_im**2 + Bz_im**2)
...
fig.savefig("EM_field_of_a_VMD_in_2D_domain.pdf",bbox_inches = 'tight', dpi=1000)