...
'''
#positions, in this example [0, range from 0 to 10000, 950]
TX_x = np.zeros(nTX).reshape(nTX,1) 
TX_y = np.arange(0., 11000.,1000.).reshape(nTX,1)
...
TX_azith_list = (90*np.ones(nTX)).reshape(nTX,1)
...
file.write(str(nRX)+'\n')
#positions, in this example [0, 0, 1000]
RX_x = np.zeros(nRX).reshape(nRX,1) 
