...
'''
#positions, in this example [0, 0, 950]
#i.e., the transmitter is deeply towed 50 meters above the seafloor.
...
#azimuths, in this example, the TX is directed in x direction
...
#from -3000 to 3000 meter, spacing 500 meter.
pos = np.arange(-3000., 3100., 500.)
...
ny = len(pos)
nz = len(pos)
RX = np.zeros((nx * ny * nz,3))

for i in range(nx):
    for j in range(ny):
        for k in range(nz):
          index = i * ny * nz + j * nz + k
          #if we generate input file following this order then index is where the data is data[i,j][k] = E[index]
          RX[index] = [x[i,j][k], y[i,j][k], z[i,j][k]]
