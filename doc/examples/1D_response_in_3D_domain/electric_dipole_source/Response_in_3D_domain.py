...
for i in range(nx):
    for j in range(ny):
        for k in range(nz):
          ...
          Ex_im[i,j][k] = EMfield.Ex_im[index]
          Ey_im[i,j][k] = EMfield.Ey_im[index]
          Ez_im[i,j][k] = EMfield.Ez_im[index]
...
ax3.quiver(x, y, z, Bx_im, By_im, Bz_im, length=0.3, normalize=True, label='Imag(B) (T)')
ax3.invert_zaxis()
...
ax4.quiver(x, y, z, Ex_im, Ey_im, Ez_im, length=0.3, normalize=True, label='Imag(E) (V/m)')
ax4.invert_zaxis()
...
fig.savefig("EM_field_of_a_HED_in_3D_domain.pdf",bbox_inches = 'tight', dpi=1000)